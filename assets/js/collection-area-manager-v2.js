( function ( $ ) {
	...
	function ensureDialog() {
		...
		host.insertAdjacentHTML( 'beforeend',
			...
			'<div class="form-hint">Detected zones are already filled in. Use Apply only when several selected accounts belong to the same zone.</div></div>' +
	...
	function renderRows() {
		...
		$( '#afc-area-manager-rows' ).html( visible.length ? visible.map( function ( user ) {
			return '<tr data-name="' + escapeAttr( user.name ) + '"><td><input class="form-check-input afc-area-user-check" type="checkbox"' +
				...
				'<td><select class="form-select form-select-sm afc-area-row-zone">' + zoneOptions( valueForUser( user ) ) + '</select></td></tr>';
