...
class Assets {
	private static function core_asset_version( $file ) {
		$modified = file_exists( $file ) ? filemtime( $file ) : false;
		return AFCN_VERSION . '-' . ( false !== $modified ? (string) $modified : '0' );
	}
	...
	public static function enqueue_core() {
		...
		$card_drag_js   = AFCN_PATH . 'assets/js/card-drag.js';
		...
		wp_enqueue_style( 'afcn-core', AFCN_URL . 'assets/css/core.css', array( 'afcn-source-serif-4' ), self::core_asset_version( $css ) );
		wp_enqueue_style( 'afcn-ui-interactions', AFCN_URL . 'assets/css/ui-interactions.css', array( 'afcn-core' ), self::core_asset_version( $interactions ) );
		wp_enqueue_style( 'afcn-module-manager', AFCN_URL . 'assets/css/module-manager.css', array( 'afcn-ui-interactions' ), self::core_asset_version( $module_manager ) );
		wp_enqueue_style( 'afcn-browser', AFCN_URL . 'assets/css/browser.css', array( 'afcn-module-manager' ), self::core_asset_version( $browser ) );
		wp_enqueue_style( 'afcn-card-order', AFCN_URL . 'assets/css/card-order.css', array( 'afcn-browser' ), self::core_asset_version( $card_order_css ) );
		wp_enqueue_script( 'afcn-ui-status', AFCN_URL . 'assets/js/ui-status.js', array(), self::core_asset_version( $status_js ), true );
		wp_enqueue_script( 'afcn-view-mode', AFCN_URL . 'assets/js/view-mode.js', array(), self::core_asset_version( $view_mode_js ), true );
		wp_enqueue_script( 'afcn-app', AFCN_URL . 'assets/js/app.js', array( 'afcn-ui-status', 'afcn-view-mode' ), self::core_asset_version( $js ), true );
		wp_enqueue_script( 'afcn-browser', AFCN_URL . 'assets/js/browser.js', array( 'afcn-app' ), self::core_asset_version( $browser_js ), true );
		wp_enqueue_script( 'afcn-card-order', AFCN_URL . 'assets/js/card-drag.js', array( 'afcn-browser' ), self::core_asset_version( $card_drag_js ), true );
		...
		if ( Capabilities::is_super_admin_user() && is_readable( $utility_js ) ) {
			wp_enqueue_script( 'afcn-utility', AFCN_URL . 'assets/js/utility.js', array( 'afcn-app' ), self::core_asset_version( $utility_js ), true );
		}
