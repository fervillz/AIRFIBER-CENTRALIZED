...
class Bootstrap {
	...
	public static function init() {
		...
		if ( defined( 'AFC_FILE' ) ) {
			register_activation_hook( AFC_FILE, array( __CLASS__, 'activate' ) );
		}
	}
	...
	public static function classic_url() {
		if ( class_exists( 'AFC_Frontend_Page' ) ) {
			return \AFC_Frontend_Page::get_url();
		}
		return admin_url( 'admin.php?page=airfiber-centralized' );
	}
			...
			if ( $existing && 'trash' !== $existing->post_status ) {
				...
				$page_id = wp_insert_post(
					array(
						'post_type'      => 'page',
						'post_status'    => 'publish',
						'post_title'     => __( 'Airfiber BETA', 'airfiber-centralized' ),
	...
	public static function protect_app() {
		...
		if ( ! User_Manager::can_access() ) {
			wp_die(
				esc_html__( 'Your account does not have permission to use Airfiber BETA.', 'airfiber-centralized' ),
	...
	public static function enqueue_assets() {
		if ( self::is_beta_request() && User_Manager::can_access() ) {
			Assets::enqueue_core();
			return;
		}
		...
		if (
			...
			&& wp_script_is( 'afc-frontend-app', 'enqueued' )
		) {
	...
	public static function render_shortcode() {
		if ( ! is_user_logged_in() || ! User_Manager::can_access() ) {
			return '';
		}
		return App::render_shell();
	}
}
