...
class Rest_Router {
	...
	public static function register_routes() {
		...
		register_rest_route(
			self::NAMESPACE_V1,
			'/module/(?P<module>[a-z0-9-]+)',
			array(
				'methods'             => \WP_REST_Server::READABLE,
				'callback'            => array( __CLASS__, 'module' ),
				'permission_callback' => array( __CLASS__, 'can_access' ),
			)
		);
		register_rest_route(
			self::NAMESPACE_V1,
			'/module/(?P<module>[a-z0-9-]+)/action/(?P<action>[a-z0-9-]+)',
			array(
				'methods'             => \WP_REST_Server::CREATABLE,
				'callback'            => array( __CLASS__, 'module_action' ),
				'permission_callback' => array( __CLASS__, 'can_access' ),
			)
		);
		register_rest_route(
			...
			array(
				'methods'             => \WP_REST_Server::CREATABLE,
				'callback'            => array( __CLASS__, 'client_metric' ),
				'permission_callback' => array( __CLASS__, 'can_access' ),
			)
		);
	}
	...
	public static function can_access() {
		return User_Manager::can_access();
	}

	public static function bootstrap() {
		return rest_ensure_response( Module_Manager::bootstrap_payload() );
	}

	public static function module( $request ) {
		$result = Module_Manager::render( $request['module'] );
	...
	public static function module_action( $request ) {
		...
		$result = Module_Manager::handle_action( $request['module'], $request['action'], $payload );
		return self::response( $result );
	}
	...
	public static function client_metric( $request ) {
		...
		if ( $duration > 0 && $duration < 60000 ) {
			Performance_Monitor::record_client( $module, $duration );
		}
