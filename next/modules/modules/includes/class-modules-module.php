	...
	public static function render( $context = array() ) {
		...
		<div class="afcn-module-list">
			<?php foreach ( $statuses as $id => $status ) :
				...
				$assets = Assets::module_manifest( $module );
				$bytes  = 0;
				foreach ( array_merge( $assets['css'], $assets['js'] ) as $asset ) {
					$bytes += isset( $asset['bytes'] ) ? (int) $asset['bytes'] : 0;
				}
				$health_variant = 'success';
				if ( 'warning' === $health['status'] ) { $health_variant = 'warning'; }
				if ( in_array( $health['status'], array( 'degraded', 'quarantined' ), true ) ) { $health_variant = 'danger'; }
			?>
				<div class="afcn-module-row">
					<div>
						<h3><?php echo esc_html( $module['name'] ); ?> <?php if ( $module['system'] ) { echo UI::badge( 'CORE', 'info' ); } // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></h3>
					...
					<div><?php echo UI::badge( strtoupper( $health['status'] ), $health_variant ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></div>
					<div class="afcn-form-actions" style="margin:0">
						<?php if ( ! $module['system'] ) : ?>
							<form data-afcn-action="toggle" data-afcn-module="modules">
								<input type="hidden" name="module_id" value="<?php echo esc_attr( $id ); ?>">
								<input type="hidden" name="enabled" value="<?php echo $status['enabled'] ? '0' : '1'; ?>">
								<button type="submit" class="afcn-button afcn-button-secondary afcn-button-small"><?php echo $status['enabled'] ? esc_html__( 'Disable', 'airfiber-centralized' ) : esc_html__( 'Enable', 'airfiber-centralized' ); ?></button>
							</form>
						<?php endif; ?>
						<?php if ( $health['violations'] > 0 ) : ?>
							<form data-afcn-action="reset-health" data-afcn-module="modules">
								<input type="hidden" name="module_id" value="<?php echo esc_attr( $id ); ?>">
								<button type="submit" class="afcn-button afcn-button-secondary afcn-button-small"><?php esc_html_e( 'Reset health', 'airfiber-centralized' ); ?></button>
							</form>
						<?php endif; ?>
					</div>
				</div>
