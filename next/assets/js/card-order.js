...
function reorder(parent,target,x,y){if(!target||target.parentElement!==parent||target===drag.card)return;const r=target.getBoundingClientRect(),before=y>=r.top&&y<=r.bottom?x<r.left+r.width/2:y<r.top+r.height/2,ref=before?target:target.nextSibling;if(ref===drag.card||(!ref&&drag.card===parent.lastElementChild))return;parent.insertBefore(drag.card,ref);changed.add(parent)}
