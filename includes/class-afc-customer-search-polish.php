...
class AFC_Customer_Search_Polish {
	...
	private static function enqueue_assets() {
		if ( wp_script_is( 'afc-customer-search-polish', 'enqueued' ) ) {
			return;
		}

		wp_enqueue_style(
			'afc-customer-search-polish',
			AFC_URL . 'assets/css/customer-search-polish.css',
		...
		wp_enqueue_script(
			'afc-customer-search-polish',
			AFC_URL . 'assets/js/customer-search-polish.js',
			array( 'jquery', 'afc-customer-actions', 'afc-ui-icons' ),
			AFC_VERSION,
			true
		);
	...
	private static function authorize() {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( array( 'message' => __( 'You do not have permission to read customer signals.', 'airfiber-centralized' ) ), 403 );
		}
	...
	private static function customer_ids( $accounts ) {
		...
		$rows = (array) $wpdb->get_results( $wpdb->prepare( $sql, $accounts ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		$out  = array();
		foreach ( $rows as $row ) {
		...
		if ( $out ) {
			update_meta_cache( 'post', array_values( $out ) );
		}
		return $out;
	}
	...
	private static function latest_jobs( $accounts ) {
		...
		$out  = array();
		foreach ( $rows as $row ) {
			$out[ strtolower( (string) $row['ppp_username'] ) ] = $row;
		}
		return $out;
	}
	...
	private static function latest_incoming( $users ) {
		...
		foreach ( $rows as $row ) {
			...
			if ( empty( $variant_map[ $phone ] ) ) {
				continue;
			}
			foreach ( $variant_map[ $phone ] as $account ) {
				if ( ! isset( $out[ $account ] ) ) {
					$out[ $account ] = $row;
				}
			}
		}
	...
	private static function signal( $user, $customer_id, $job, $incoming ) {
		...
		if ( $customer_id ) {
			if ( '' === $next ) {
				$next = trim( (string) get_post_meta( $customer_id, '_afc_comment_field_nextdue', true ) );
			}
			if ( '' === $cutoff ) {
				$cutoff = trim( (string) get_post_meta( $customer_id, '_afc_comment_field_cutoffdate', true ) );
			}
		}
		...
		$expired        = 0 === strcasecmp( $user['actual_profile'], 'Expired' ) || ( null !== $days_to_cutoff && $days_to_cutoff < 0 );
		...
		if ( $expired ) {
			...
			$due_state = 'soon';
		} elseif ( null !== $days_to_cutoff && $days_to_cutoff <= 3 ) {
			$due_state = 'soon';
		} elseif ( ( null !== $days_to_due && $days_to_due <= 7 ) || ( null !== $days_to_cutoff && $days_to_cutoff <= 7 ) ) {
			$due_state = 'upcoming';
		} elseif ( null !== $days_to_due || null !== $days_to_cutoff ) {
			$due_state = 'safe';
		}
