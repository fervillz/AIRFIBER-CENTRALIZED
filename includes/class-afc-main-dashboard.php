...
class AFC_Main_Dashboard {
	...
	public static function enqueue_assets() {
		...
		wp_enqueue_style(
			'afc-main-dashboard',
			AFC_URL . 'assets/css/main-dashboard.css',
			array( 'afc-frontend-app', 'afc-basic-payments' ),
			AFC_VERSION
		);

		wp_enqueue_script(
			'afc-main-dashboard',
			AFC_URL . 'assets/js/main-dashboard.js',
			array( 'afc-frontend-app', 'afc-basic-payments', 'afc-sortablejs' ),
			AFC_VERSION,
			true
		);
	...
	private static function drag_handle() {
		return '<button type="button" class="afc-dashboard-drag" aria-label="' . esc_attr__( 'Drag to reorder this card', 'airfiber-centralized' ) . '" title="' . esc_attr__( 'Drag to reorder', 'airfiber-centralized' ) . '"><span></span><span></span><span></span><span></span><span></span><span></span></button>';
	}

	private static function card_header( $kicker, $title, $icon ) {
		return '<header class="afc-dashboard-card-head"><div class="afc-dashboard-card-title"><span class="afc-dashboard-card-icon" aria-hidden="true">' . esc_html( $icon ) . '</span><div><small>' . esc_html( $kicker ) . '</small><h2>' . esc_html( $title ) . '</h2></div></div>' . self::drag_handle() . '</header>';
	}

	private static function render_widget( $id ) {
		switch ( $id ) {
			case 'payment':
				?>
				<article class="afc-dashboard-card afc-dashboard-payment-card" data-afc-dashboard-widget="payment">
					<?php echo self::card_header( __( 'Most used', 'airfiber-centralized' ), __( 'Record a payment', 'airfiber-centralized' ), '₱' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
			...
			case 'new-ppp':
				...
				<article class="afc-dashboard-card afc-dashboard-new-ppp" data-afc-dashboard-widget="new-ppp">
					<?php echo self::card_header( __( 'Quick action', 'airfiber-centralized' ), __( 'New PPP account', 'airfiber-centralized' ), '+' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
					<div class="afc-dashboard-new-ppp-body">
						<div class="afc-dashboard-new-ppp-mark" aria-hidden="true">+</div>
						<p><?php esc_html_e( 'Add a new subscriber, billing details, plan, Wi-Fi details, and MikroTik PPP secret.', 'airfiber-centralized' ); ?></p>
			...
			case 'network':
				...
				<article class="afc-dashboard-card afc-dashboard-network-card" data-afc-dashboard-widget="network">
					<?php echo self::card_header( __( 'Live router snapshot', 'airfiber-centralized' ), __( 'ISP ports & router health', 'airfiber-centralized' ), '⌁' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
					<div class="afc-dashboard-router-metrics" data-afc-dashboard-router-metrics>
						<div class="afc-dashboard-skeleton-line is-wide"></div>
					</div>
	...
	private static function render_list_card( $id, $kicker, $title, $icon, $group, $panel ) {
		...
		<article class="afc-dashboard-card afc-dashboard-list-card" data-afc-dashboard-widget="<?php echo esc_attr( $id ); ?>">
			<?php echo self::card_header( $kicker, $title, $icon ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
			...
			<footer class="afc-dashboard-card-footer"><span data-afc-dashboard-count="<?php echo esc_attr( $group ); ?>">—</span><button type="button" data-afc-dashboard-open-panel="<?php echo esc_attr( $panel ); ?>"><?php esc_html_e( 'Open', 'airfiber-centralized' ); ?> →</button></footer>
		</article>
		<?php
	}
		...
		<section class="afc-frontend-panel afc-dashboard-panel" data-afc-panel="dashboard" aria-hidden="true" hidden>
			<div class="afc-dashboard-shell" id="afc-main-dashboard">
				...
				<div class="afc-dashboard-grid" data-afc-dashboard-grid>
					<?php foreach ( $layout as $widget_id ) : ?>
	...
	private static function scheduler_map( $rows ) {
		$map = array();
		foreach ( $rows as $row ) {
	...
	private static function account_item( $secret, $scheduler ) {
		...
		return array(
			...
			'customer'      => ! empty( $details['name'] ) ? (string) $details['name'] : ( isset( $secret['name'] ) ? (string) $secret['name'] : '' ),
	...
	private static function account_groups( $secrets, $schedulers ) {
		...
		$map             = self::scheduler_map( $schedulers );
		$timezone        = function_exists( 'wp_timezone' ) ? wp_timezone() : new DateTimeZone( 'UTC' );
		...
		foreach ( $secrets as $secret ) {
			...
			$scheduler = $username && isset( $map[ strtolower( $username ) ] ) ? $map[ strtolower( $username ) ] : null;
			$item      = self::account_item( $secret, $scheduler );
			if ( ! $item['name'] ) {
				continue;
			}

			$due = self::parse_date( $item['nextDue'] );
			if ( $due && 0 !== strcasecmp( $item['profile'], $expired_profile ) ) {
				$days = self::days_between( $today, $due );
				if ( $days >= 0 && $days <= 7 ) {
					$item['daysUntil'] = $days;
					$groups['due'][]   = $item;
				}
			...
			if ( 0 === strcasecmp( $item['profile'], $expired_profile ) ) {
				...
				$item['daysAgo']     = $cutoff ? max( 0, - self::days_between( $today, $cutoff ) ) : null;
				$groups['expired'][] = $item;
			}

			$payment = self::parse_date( $item['paymentDate'] );
			if ( $payment ) {
				$item['daysAgo']      = max( 0, - self::days_between( $today, $payment ) );
				$groups['payments'][] = $item;
			}

			$installed = self::parse_date( $item['installed'] );
			if ( $installed ) {
				$item['daysAgo']     = max( 0, - self::days_between( $today, $installed ) );
	...
	private static function sms_data() {
		...
		$count = (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$table} WHERE DATE(created_at) = CURDATE()" ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		return array( 'items' => is_array( $rows ) ? $rows : array(), 'count' => $count );
	}
	...
	private static function network_data( $resource ) {
		...
		$first   = self::rows( AFC_MikroTik::run_command( $command ) );
		if ( is_wp_error( $first ) ) {
			return array( 'ports' => array(), 'error' => $first->get_error_message() );
		}
		usleep( 650000 );
		$second = self::rows( AFC_MikroTik::run_command( $command ) );
		...
		for ( $index = 1; $index <= 8; $index++ ) {
			...
			$ports[] = array(
				...
				'found'     => (bool) $key,
				'running'   => $key && in_array( strtolower( (string) ( isset( $row['running'] ) ? $row['running'] : '' ) ), array( 'true', 'yes', '1' ), true ),
				'disabled'  => $key && in_array( strtolower( (string) ( isset( $row['disabled'] ) ? $row['disabled'] : '' ) ), array( 'true', 'yes', '1' ), true ),
		...
		$memory_used  = $total_memory > 0 ? round( ( ( $total_memory - $free_memory ) / $total_memory ) * 100 ) : 0;
		return array(
			'ports'      => $ports,
	...
	public static function ajax_data() {
		...
		$secrets = self::rows( AFC_MikroTik::run_command( array( '/ppp/secret/print', '=.proplist=name,profile,comment,disabled' ) ) );
		$schedulers = self::rows( AFC_MikroTik::run_command( array( '/system/scheduler/print', '=.proplist=name,start-date,start-time,disabled,run-count,next-run' ) ) );
		$groups = ( is_wp_error( $secrets ) || is_wp_error( $schedulers ) )
		...
		$groups['groups']['sms'] = $sms['items'];
		$groups['counts']['sms'] = $sms['count'];
