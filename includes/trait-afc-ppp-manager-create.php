...
trait AFC_PPP_Manager_Create_Trait {
	public static function ajax_create() {
		...
		$wifi          = substr( sanitize_text_field( isset( $_POST['wifi'] ) ? wp_unslash( $_POST['wifi'] ) : '' ), 0, 120 );
		$wifi_password = isset( $_POST['wifi_password'] ) ? (string) wp_unslash( $_POST['wifi_password'] ) : '';
		$wifi_password = preg_replace( '/[\x00-\x1F\x7F]/', '', $wifi_password );
		$wifi_password = substr( trim( $wifi_password ), 0, 120 );
		...
		if ( 'basic' === $mode && ( ! $wifi || strlen( $wifi_password ) < 8 ) ) {
			wp_send_json_error( array( 'message' => __( 'WiFi name and a WiFi password with at least 8 characters are required.', 'airfiber-centralized' ) ) );
		}
		...
		if ( 'basic' === $mode ) {
			$comment_values['password'] = $wifi_password;
		}
		$comment = self::build_comment( '', $comment_values );
