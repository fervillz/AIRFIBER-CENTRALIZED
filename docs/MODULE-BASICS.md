...
If declared in `module.json`, module assets are loaded when that module opens; they are not sent on the initial Airfiber app request simply because the module is enabled.
...
- `render_chunk()` for a lazy HTML section.
...
Load real work when the user opens the feature, requests a lazy chunk/query, or when a background task intentionally runs.
...
2. Connector/menu/dependency metadata can be compiled into a cheap registry.
